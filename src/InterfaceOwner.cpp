...
Interface* InterfaceOwner::getReferenceType() const {
  return mInterface;
}
...
bool InterfaceOwner::canCastTo(IRGenerationContext& context, const IType* toType) const {
  ...
  if (toType->isOwner()) {
    return mInterface->canCastTo(context, toType->getReferenceType());
  }
...
Value* InterfaceOwner::castTo(IRGenerationContext& context,
  ...
  if (toType->isOwner()) {
    return mInterface->castTo(context, fromValue, toType->getReferenceType(), line);
  }

//
//  TestReminderExpression.cpp
//  runtests
//
//  Created by Vladimir Fridman on 8/5/18.
//  Copyright © 2018 Vladimir Fridman. All rights reserved.
//
//  Tests {@link ReminderExpression}
//
...
struct ReminderExpressionTest : Test {
...
TEST_F(ReminderExpressionTest, isConstantTest) {
  ReminderExpression expression(mLeft, mRight, 11);
  
  EXPECT_FALSE(expression.isConstant());
}

TEST_F(ReminderExpressionTest, isAssignableTest) {
  ReminderExpression expression(mLeft, mRight, 11);
  
  EXPECT_FALSE(expression.isAssignable());
}

TEST_F(ReminderExpressionTest, generateIRTest) {
  ReminderExpression expression(mLeft, mRight, 11);
  ...
  *mStringStream << instruction;
  ASSERT_STREQ("  %rem = srem i32 3, 5", mStringStream->str().c_str());
}

TEST_F(ReminderExpressionTest, printToStreamTest) {
  ReminderExpression expression(mLeft, mRight, 11);
...
TEST_F(ReminderExpressionTest, incompatibleTypesDeathTest) {
...
TEST_F(ReminderExpressionTest, nonPrimitiveTypesDeathTest) {
  ...
  ReminderExpression expression(mLeft, mRight, 9);
...
TEST_F(ReminderExpressionTest, voidTypesDeathTest) {
  Mock::AllowLeak(mLeft);
  Mock::AllowLeak(mRight);
  ...
  ReminderExpression expression(mLeft, mRight, 5);
...
TEST_F(ReminderExpressionTest, explicitCastNeededOnGenerateIRDeathTest) {
  Mock::AllowLeak(mLeft);
  Mock::AllowLeak(mRight);
  ...
  ReminderExpression expression(mLeft, mRight, 3);
...
TEST_F(ReminderExpressionTest, explicitCastNeededOnGetTypeDeathTest) {
  Mock::AllowLeak(mLeft);
  Mock::AllowLeak(mRight);
  ...
  ReminderExpression expression(mLeft, mRight, 1);

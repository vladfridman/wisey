...
TEST_F(LongTypeTest, canAutoCastToTest) {
  ...
  EXPECT_FALSE(mLongType.canAutoCastTo(mContext, PrimitiveTypes::BYTE));
...
TEST_F(LongTypeTest, canCastTest) {
  ...
  EXPECT_TRUE(mLongType.canCastTo(mContext, PrimitiveTypes::BYTE));
...
TEST_F(LongTypeTest, castToTest) {
  ...
  *mStringStream << *result;
  ...
  result = mLongType.castTo(mContext, expressionValue, PrimitiveTypes::BYTE, 0);
  *mStringStream << *result;
...
TEST_F(TestFileRunner, signedCastLongToIntRunTest) {
  runFile("tests/samples/test_signed_cast_long_to_int.yz", -3);
}
